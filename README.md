...
and `python figures/make_all.py` regenerates the nine legacy figures (~1–4 min)
deterministically from the master seeds. The paper figure
(`figures/exp1_crossover.png`) is produced separately by
...
| `figures/` | nine legacy figures regenerated by `make_all.py` from the master seeds, plus the paper figure `exp1_crossover.png` (from `experiments/cp1_analysis.py`) and `exp1_crossover_2panel.png` (the figure embedded in the CN2026 submission) |
